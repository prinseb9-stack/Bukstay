...
export default function UserMessages() {
  ...
  const filteredChats = chats.filter(chat => {
    ...
    return (
      chat.hostName?.toLowerCase().includes(term) ||
  ...
  return (
    <div className={`${isDark ? 'bg-[#0f0f1a]' : 'bg-gray-50'} min-h-screen`}>
      <div className={`${isDark ? 'bg-[#1a1a2e]' : 'bg-white'} p-4 sticky top-0 z-10 border-b ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
        <h1 className={`text-xl font-bold mb-3 ${isDark ? 'text-white' : 'text-black'}`}>Messages</h1>
      ...
      <div className="divide-y" style={{ borderColor: isDark ? '#1f1f2e' : '#f0f0f0' }}>
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <h2 className={`text-lg font-bold mb-2 ${isDark ? 'text-white' : 'text-black'}`}>No messages</h2>
            <p className={`text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Messages with hosts will appear here</p>
          </div>
        ) : (
          filteredChats.map(chat => (
            <div key={chat.id} onClick={() => navigate(`/User/messages/${chat.id}`)}
              className={`flex items-center gap-3 p-4 cursor-pointer ${isDark ? 'hover:bg-[#1a1a2e]' : 'hover:bg-gray-50'}`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex justify-between">
                  <h3 className={`font-semibold truncate ${isDark ? 'text-white' : 'text-black'}`}>{chat.hostName}</h3>
                  <span className={`text-xs ${isDark ? 'text-gray-500' : 'text-gray-400'}`}>{formatTime(chat.updatedAt)}</span>
                </div>
